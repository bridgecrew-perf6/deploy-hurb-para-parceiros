...
$(document).ready(function() {
    ...
    var settings = {
        "url": `https://hurb-para-parceiros.herokuapp.com/api/v1/user/u/${sessionStorage.getItem("target-edit-id")}`,
        "method": "GET",
        "timeout": 0,
        "headers": {
    ...
    $.ajax(settings).done(function (response) {
         ...
         var settings = {
          ...
          document.querySelector(".primary-button").addEventListener("click", function() {
            var settings = {
                "url": `https://hurb-para-parceiros.herokuapp.com/api/v1/user/u/${user.id}`,
                "method": "PUT",
                "timeout": 0,
                "headers": {
                  "Authorization": localStorage.getItem("token"),
